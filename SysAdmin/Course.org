...
Startup was viewed as a serial process, divided into a series of sequential stages (termed run levels). Each stage required completion before the next could proceed.
...
Every other major Linux distribution has now adopted ~systemd~ and configured it as the default.
Even Ubuntu phased out ~Upstart~ in its favor.
...
In the following, we will concentrate just on ~systemd~. For the most part, we will ignore ~SysVinit~ as well as ~Upstart~, which is no longer used in any significant way.
** systemd Features
...
Granular logging is provided by default using the name of the unit.
Tracking and creating user processes without root privileges is another powerful ability of ~systemd~.
** systemd Configuration File Locations
~systemd~ allows for multiple locations for its unit files, allowing for processes running under a non-root user, and inheritance from system unit files.
Here are some typical locations (but may vary by distribution):
...
ExecReload=/usr/bin/dbus-send --print-reply --system --type=method_call --dest=org.freedesktop.DBus / org.freedesktop.DBus.ReloadConfig OOMScoreAdjust=-900
#+end_src
Unit files can be quite complex. In this example, you can see a way to include a link to documentation and different methods for starting and restarting the service.
** systemctl
...
#+end_src
We limited our output to only units starting with xdg under a desktop Xorg session in the command above because there are a large number of user units too.
...
Re-creating lost data costs time and money. Some data may even be unique and there may be no way to re-create it.

While storage media reliability has increased, so has drive capacity. Even if the failure rate per byte decreases, unpredictable failures still occur.
It may be pessimistic to say there are only two kinds of drives; those that have failed and those that will fail, but it is essentially true. Using /RAID/ helps, but backups are still needed.

No software is perfect. Bugs may destroy or corrupt data. Even stable programs long in use can have problems.

Everyone has heard OOPS! (or something much worse) coming from the next cubicle (or from their own mouth) at one time or another.
...
Files can just disappear without you knowing how, who, or even when it occurred. Sometimes restoring to an earlier snapshot of all or part of the system may be required.
...
Also, the swap partitions (or files) and =/proc= filesystems are generally not useful or necessary to back up, since the data in these areas is basically temporary (just like in the =/tmp= directory).
** Backup vs. Archive
...
These lifetimes is very sensitive to:
...
** Tape Drives
Tape drives are not as common as they used to be. They are relatively slow and permit only sequential access.
On any modern setup, they are rarely used for primary backup. They are sometimes used for off-site storage for archival purposes, for long time reference.
However, magnetic tape drives always have only a finite lifetime without physical degradation and loss of data.

Modern tape drives are usually of the /LTO/ (Linear Tape Open) variety, whose first versions appeared in the late 1990s as an open standards alternative; early formats were mostly proprietary.
Early versions held up to 100 GB; newer versions can hold 2.5 TB or more in a cartridge of the same size.
...
** Backup Methods
You should never have all backups residing in the same physical location as the systems being protected. Otherwise, fire or other physical damage could lead to a total loss. In the past, this usually meant physically transporting magnetic tapes to a secure location.
Today, this is more likely to mean transferring backup files over the Internet to alternative physical locations. Obviously, this has to be done in a secure way, using encryption and other security precautions as is appropriate.
...
The archives are often compressed with /gzip/, /bzip2/, or /xz/.
...
Below are a few examples of how to use the tar command for backups.
...
The ~-x~ or ~--extract~ option extracts files from an archive, all by default. You can narrow the file extraction list by specifying only particular files.
...
Either option requires specifying either a date or a qualified (reference) file name. See commands below:
#+begin_src bash
...
When followed by an option like ~--newer~ you must use the dash in options like ~-vzf~, or tar will get confused.
This kind of option specification confusion sometimes occurs with old UNIX utilities like ~ps~ and ~tar~ with complicated histories involving different families of UNIX.
...
Note that the first command above has the exact same effect as doing:
#+begin_src bash
...
~rsync~ is very clever; it checks local files against remote files in small chunks, and it is very efficient in that when copying one directory to a similar directory, only the differences are copied over the network.
This synchronizes the second directory with the first directory. You may often use the ~-r~ option, which causes ~rsync~ to recursively walk down the directory tree copying all files and directories below the one listed as the /sourcefile/.
Thus, a very useful way to back up a project directory might be similar to this command:
#+begin_src bash
...
+ Amanda
  [[https:amanda.org/][Amanda]] (Advanced Maryland Automatic Network Disk Archiver) uses native utilities (including ~tar~ and ~dump~), but is far more robust and controllable.
  Amanda is generally available on Enterprise Linux systems through the usual repositories.
+ Bacula
  [[https:bacula.org/7.0.x-manuals/en/main/Main_Reference.html][Bacula]] is designed for automatic backup on heterogeneous networks.
...
+ Clonezilla
  [[https:clonezilla.org/][Clonezilla]] is a very robust disk cloning program, which can make images of disks and deploy them, either to restore a backup, or to be used for /ghosting/, to provide an image that can be used to install many machines.
...
Responsibility for enabling good security controls falls both on application designers and the Linux kernel developers and maintainers. Of course, users have to follow good procedures as well, but on a well-run system, non-privileged users should have very limited ability to expose the system to security violations.
...
The idea is to implement mandatory access controls on requests made to the kernel, but to do so in a way that:
...
The basic idea is to hook system calls; insert code whenever an application requests a transition to kernel (system) mode in order to accomplish work that requires enhanced abilities; this code makes sure permissions are valid, malicious intent is protected against, etc.
...
+ [[https:selinuxproject.org/page/Main_Page][SELinux]]
+ [[https:gitlab.com/apparmor][AppArmor]]
+ [[https:schaufler-ca.com/][Smack]]
+ [[https:tomoyo.sourceforge.net/index.html.en][Tomoyo]]
+ [[https:kernel.org/doc/html/latest/admin-guide/LSM/Yama.html][Yama]]
We will concentrate primarily on ~SELinux~ and secondarily on ~AppArmor~ in order of usage volume.

See "[[https:starlab.io/blog/a-brief-tour-of-linux-security-modules][A Brief Tour of Linux Security Modules]]" by Shaun Ruffell for a nice review of the mechanism and choices.
** SELinux Overview
...
+ Contexts
  These are labels to files, processes, and ports. Examples of contexts are SELinux user, role and type.
+ Rules
  They describe access control in terms of contexts, processes, files, ports, users, etc.
+ Policies
  They are a set of rules that describe what system-wide access control decisions should be made by SELinux.
...
SELinux can be run under one of three modes. These modes are selected (and explained) in a file (usually =/etc/selinux/config=) whose location varies by distribution (it is often either at =/etc/sysconfig/selinux= or linked from there). This file is well documented.
+ Enforcing
  All SELinux code is operative and access is denied according to policy. All violations are audited and logged.

  [[file:images/SELinus_Mode_Enforcing.png]]

+ Permissive
  Enables SELinux code, but only audits and warns about operations that would be denied in enforcing mode.

  [[file:images/SELinux_Mode_Permissive.png]]
...
To examine or set the current mode, you can use ~getenforce~ and ~setenforce~ (commands and outputs below):
#+begin_src bash
$ getenforce
Enforcing
...
+ Configuration file
  Edit the SELinux configuration file (usually =/etc/selinux/config=) and set ~SELINUX=disabled~.
...
The most common SELinux policies are:
+ targeted
  The default policy in which SELinux is more restricted to targeted processes.
...
We will focus on /type/, the most commonly utilized context.
...
Often the parameter ~Z~ is passed to standard command line tools, as in (commands and outputs below):
#+begin_src bash
$ ps axZ
...
Continuing the previous example, we see the context of /tmpfile/ was not changed by moving the file from =/tmp= to =/home/jimih= (commands and outputs below):
#+begin_src bash
$ cd /tmp/
$ touch tmpfile
$ ls -Z tmpfile
...
Creating a file in the =/tmp= directory, and then moving it to the =DocumentRoot= directory, will make the file inaccessible to the /httpd/ server until the SELinux context of the file is adjusted.
** restorecon
~restorecon~ resets file contexts, based on parent directory settings.
In the following example, ~restorecon~ resets the default label recursively for all files at the home directory (commands and outputs below):
#+begin_src bash
$ ls -Z
-rw-rw-r--. jimih jimih unconfined_u:object_r:user_home_t:s0 homefile
-rw-rw-r--. jimih jimih unconfined_u:object_r:user_tmp_t:s0 tmpfile
...
Note that ~semanage fcontext~ only changes the default settings; it does not apply them to existing objects. This requires calling ~restorecon~ afterwards. For example:
#+begin_src bash
...
# calling restorecon to apply changes
...
#+end_src
** AppArmor
/AppArmor/ is an LSM alternative to SELinux. Support for it has been incorporated in the Linux kernel since 2006.
It has been used by SUSE, Ubuntu and other distributions. It does the following:
...
+ Enforce Mode
  Applications are prevented from acting in ways which are restricted.
...
+ Complain Mode
  Policies are not enforced, but attempted policy violations are reported.
  This is also called the /learning mode/. A profile can be set to this mode with ~aa-complain~.
...
For example, on an openSUSE system (command and output below):
#+begin_src bash
...
Note that many of these utilities can be invoked with either their short or long names; e.g. (command and output below):
#+begin_src bash
